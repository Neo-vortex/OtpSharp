...
{
    [Theory]
    [InlineData(OtpSharp.Algorithms.OtpAlgorithm.HmacSha1,   20)]
    [InlineData(OtpSharp.Algorithms.OtpAlgorithm.HmacSha256, 32)]
    [InlineData(OtpSharp.Algorithms.OtpAlgorithm.HmacSha512, 64)]
    public void GenerateForAlgorithm_ReturnsCorrectLength(
        OtpSharp.Algorithms.OtpAlgorithm algorithm, int expectedBytes)
    {
...
public class TotpValidationServiceTests
{
    ...
    public void Service_WithCustomOptions()
    {
        var service = new TotpValidationService(b => b
            .WithDigits(8)
            .WithAlgorithm(OtpSharp.Algorithms.OtpAlgorithm.HmacSha256));

        service.Options.Digits.Should().Be(8);
        service.Options.Algorithm.Should().Be(OtpSharp.Algorithms.OtpAlgorithm.HmacSha256);
    }
}
